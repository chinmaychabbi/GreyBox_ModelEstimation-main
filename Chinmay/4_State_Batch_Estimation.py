# -*- coding: utf-8 -*-
"""4_Batch_Estimation.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1b86vL0xRPaw_gMcsGn7D4fJ2XxIvAI6Z
"""

!pip install casadi

## Importing external modules

import numpy as np
import matplotlib.pyplot as plt

from casadi import *

## Initial Setup

# State/Parameter/Output Dimensions
State_n = 4
Parameter_n = 12
Output_n = 1
Input_n = 8

# Initial Filter stae mean/covariance - as one state
T_ave = 22. #room temp
T_wall = 24.
T_attic = 22.
T_im = 22.

#parameters

R_win = 0
R_w2 = 0
R_attic = 0
R_im = 0
R_roof = 0
C_in = 0
C_w = 0
C_attic = 0
C_im = 0
C1 = 0
C2 = 0
C3 = 0


# State Covariance
P_model = 1

# Filter process/measurement noise covariances
Q_model = 0.01
Q_params = 0.01
R_model = 0.01

# Creating Infinity
Infinity = np.inf

## Getting total time steps
N = y_measured_noisy.shape[1]

## Creating Optimization Variables

# State Variables
T_ave_l = SX.sym('T_ave_l',N,1)
T_wall_l = SX.sym('T_wall_l',N,1)
T_attic_l = SX.sym('T_attic_l',N,1)
T_im_l = SX.sym('T_im_l',N,1)

# Parameter Variables
R_win = SX.sym('R_win',1,1)
R_w2 = SX.sym('R_w2',1,1)
R_attic = SX.sym('R_attic',1,1)
R_im = SX.sym('R_im',1,1)
R_roof = SX.sym('R_roof',1,1)
C_in = SX.sym('C_in',1,1)
C_w = SX.sym('C_w',1,1)
C_attic = SX.sym('C_attic',1,1)
C_im = SX.sym('C_im',1,1)
C1 = SX.sym('C1',1,1)
C2 = SX.sym('C2',1,1)
C3 = SX.sym('c3',1,1)

# Other Variables
v_l = SX.sym('v_l',N,1)
w_T_ave_l = SX.sym('w_T_ave_l',N-1,1)
w_T_wall_l = SX.sym('w_T_wall_l',N-1,1)
w_T_attic_l = SX.sym('T_attic_l',N,1)
w_T_im_l = SX.sym('T_im_l',N,1)

## Constructing the Cost Function

# Constructing P_1_0 Matrix
P_1_0_inv = np.reshape(np.diag(np.reshape((1/P_model)*np.ones(State_n,),(State_n,)),k=0),(State_n,State_n))

##########################################################
# Constructing Intermediate Variables
X_1_e = vcat([T_ave_l[0] - T_ave] , [T_wall_l[0] - T_wall] , [T_attic_l[0] - T_attic] , [T_im_l[0] - T_im])

#############################################################
w_l = vcat([w_T_ave_l, w_T_wall_l, w_T_attic_l, w_T_im_l])


# Cost Function Development
CostFunction = 0

# c X_1_e_T * P_1_0_inv * X_1_e
CostFunction += X_1_e.T @ P_1_0_inv @ X_1_e

# Initializing Lower-Upper Bounds for State/Parameters/Intermediate variables and the Equations

T_ave_lb  = [0]
T_ave_ub  = [np.inf]

T_wall_lb  = [0]
T_wall_ub  = [np.inf]

T_attic_lb  = [0]
T_attic_ub  = [np.inf]

T_im_lb  = [0]
T_im_ub  = [np.inf]

R_win_lb = [0]
R_win_ub = [np.inf]

R_w2_lb = [0]
R_w2_ub = [np.inf]

R_attic_lb = [0]
R_attic_ub = [np.inf]

R_im_lb = [0]
R_im_ub = [np.inf]

R_roof_lb = [0]
R_roof_ub = [np.inf]

C_in_lb= [0]
C_in_ub = [np.inf]

C_w_lb= [0]
C_w_ub = [np.inf]

C_attic_lb= [0]
C_attic_ub = [np.inf]

C_im_lb= [0]
C_im_ub = [np.inf]

C1_lb = [0]
C1_ub = [np.inf]

C2_lb = [0]
C2_ub = [np.inf]

C3_lb = [0]
C3_ub = [np.inf]

v_lb = []
v_ub = []

w_T_ave_lb = []
w_T_ave_ub = []

w_T_wall_lb = []
w_T_wall_ub = []

w_T_attic_lb = []
w_T_attic_ub = []

w_T_im_lb = []
w_T_im_ub = []

Eq_T_ave_lb = []
Eq_T_wall_lb = []
Eq_T_attic_lb = []
Eq_T_im_lb = []

Eq_R_win_lb = []
Eq_R_w2_lb = []
Eq_R_attic_lb = []
Eq_R_im_lb = []
Eq_R_roof_lb = []

Eq_C_in_lb= []
Eq_C_w_lb= []
Eq_C_attic_lb= []
Eq_C_im_lb= []
Eq_C1_lb = []
Eq_C2_lb = []
Eq_C3_lb = []

Eq_y_lb = []

Eq_T_ave_ub = []
Eq_T_wall_ub = []
Eq_T_attic_ub = []
Eq_T_im_ub = []

Eq_R_win_ub = []
Eq_R_w2_ub = []
Eq_R_attic_ub = []
Eq_R_im_ub = []
Eq_R_roof_ub = []

Eq_C_in_ub= []
Eq_C_w_ub= []
Eq_C_attic_ub= []
Eq_C_im_ub= []
Eq_C1_ub = []
Eq_C2_ub = []
Eq_C3_ub = []

#Stacking Equations

Eq_T_ave = []
Eq_T_wall = []
Eq_T_attic = []
Eq_T_im = []

Eq_R_win = []
Eq_R_w2 = []
Eq_R_attic = []
Eq_R_im = []
Eq_R_roof = []

Eq_C_in = []
Eq_C_w = []
Eq_C_attic = []
Eq_C_im = []
Eq_C1 = []
Eq_C2 = []
Eq_C3 = []
Eq_y = []

# FOR LOOP: For each time step
for ii in range(N):

    # Computing Cost Function: v_l_T * R_inv * v_l
    CostFunction += v_l[ii]**2 * (1/R_model)

    if (ii < N-1):

      # Computing Cost Function: w_l_T * Q_inv * w_l
        CostFunction += w_T_ave_l[ii]**2 * (1/Q_model)

        CostFunction += w_T_wall_l[ii]**2 * (1/Q_model)

        CostFunction += w_T_attic_l[ii]**2 * (1/Q_model)

        CostFunction += w_T_im_l[ii]**2 * (1/Q_model)

        # State Equations - Formulation
##########################
        T_ave_Eq = -T_ave_l[ii+1] + T_ave_l[ii] + ts*(((T_wall[] - T_ave[])/(C_in * (R_w/2))) + ((T_am[] - T_ave[])/(C_in * (R_win))) + ((T_attic[] - T_ave[])/(C_in * (R_attic))) + ((T_im[] - T_ave[])/(C_in * (R_im))) + ((C1 * Q_in[])/C_in) + (( C2 * Q_ac[])/C_in ) + ( Q_venti[ii]) + (Q_infi[ii]) ) + w_T_ave_l[ii]
        # ts(x2_l[ii])= ( ((T_wall[] - T_ave[])/(C_in * (R_w/2))) + ((T_am[] - T_ave[])/(C_in * (R_win))) + ((T_attic[] - T_ave[])/(C_in * (R_attic))) + ((T_im[] - T_ave[])/(C_in * (R_im))) + ((C1 * Q_in[])/C_in) + (( C2 * Q_ac[])/C_in ) + ( Q_venti[ii]) + (Q_infi[ii]) )

##########################
        T_wall_Eq = -T_wall_l[ii+1] + T_wall_l[ii] + ts*(1/((C_w) * (R_w/2)) ( T_ave[ii] - T_wall[ii] + T_sol[ii] - T_wall[ii] )) + w_T_wall_l[ii]
        #ts*()=  (1/((C_w) * (R_w/2)) ( T_ave[ii] - T_wall[ii] + T_sol[ii] - T_wall[ii] ) )

##########################

        T_attic_Eq = -T_ave_l[ii+1] + T_ave_l[ii] + ts*((1/C_attic) * ((T_ave[ii] - T_attic[ii] / R_attic ) + (T_sol[ii] - T_attic[ii] / R_roof ))) + w_T_ave_l[ii]
        #ts*()=  ((1/C_attic) * ((T_ave[ii] - T_attic[ii] / R_attic ) + (T_sol[ii] - T_attic[ii] / R_roof )))

##########################
        T_im_Eq = -T_wall_l[ii+1] + T_wall_l[ii] + ts*(( 1/C_im ) * (((T_ave[ii]-T_im[ii])/R_im) + (C3 * Q_sol[ii]))) + w_T_wall_l[ii]
        #ts*() = ( 1/C_im ) * (((T_ave[ii]-T_im[ii])/R_im) + (C3 * Q_sol[ii]))


        # Adding current equations to Equation List
        Eq_T_ave += [T_ave_Eq]
        Eq_T_wall += [T_wall_Eq]
        Eq_T_attic += [T_attic_Eq]
        Eq_T_im += [T_im_Eq]

        # Adding Equation Bounds
        Eq_T_ave_lb += [0]
        Eq_T_wall_lb += [0]
        Eq_T_attic_lb += [0]
        Eq_T_im_lb += [0]

        Eq_T_ave_ub += [0]
        Eq_T_wall_ub += [0]
        Eq_T_attic_ub += [0]
        Eq_T_im_ub += [0]

         # Adding Variable Bounds
        w_T_ave_lb += [-Infinity]
        w_T_ave_ub += [Infinity]

        w_T_wall_lb += [-Infinity]
        w_T_wall_ub += [Infinity]

        w_T_attic_lb += [-Infinity]
        w_T_attic_ub += [Infinity]

        w_T_im_lb += [-Infinity]
        w_T_im_ub += [Infinity]

#########################
    # Output Equations - Formulation
    #Equation remains same for 1/2/4
    y_Eq = -v_l[ii] + y_measured[ii] - T_ave_l[ii]

    # Adding current equations to Equation List
    Eq_y += [y_Eq]

    # Adding Equation Bounds
    Eq_y_lb += [0]

    Eq_y_ub += [0]

    # Adding Variable Bounds
    T_ave_lb += [-Infinity]
    T_ave_ub += [Infinity]

    T_wall_lb += [-Infinity]
    T_wall_ub += [Infinity]

    T_attic_lb += [-Infinity]
    T_attic_ub += [Infinity]

    T_im_lb += [-Infinity]
    T_im_ub += [Infinity]

    v_lb += [-Infinity]
    v_ub += [Infinity]

## Constructing NLP Problem

# Creating Optimization Variable: x
x = vcat([T_ave_l, T_wall_l, T_attic_l, T_im_l, v_l, w_T_ave_l, w_T_wall_l, w_T_attic_l, w_T_im_l, R_win , R_w2 , R_attic , R_im , R_roof , C_in, C_w , C_attic, C_im , C1, C2, C3])

# Creating Cost Function: J
J = CostFunction

# Creating Constraints: g
g = vertcat(*Eq_T_ave, *Eq_T_wall, *Eq_T_attic, *Eq_T_im, *Eq_y)

# Creating NLP Problem
NLP_Problem = {'f': J, 'x': x, 'g': g}

## Constructiong NLP Solver
NLP_Solver = nlpsol('nlp_solver', 'ipopt', NLP_Problem)

## Solving the NLP Problem

# Creating Initial Variables
T_ave_l_ini = (T_ave * np.ones((N+1))).tolist()
T_wall_l_ini = (T_wall * np.ones((N+1))).tolist()
T_attic_l_ini = (T_attic * np.ones((N+1))).tolist()
T_im_l_ini = (T_im * np.ones((N+1))).tolist()

v_l_ini = np.zeros((N,)).tolist()
w_T_ave_l_ini = np.zeros((N-1,)).tolist()
w_T_wall_l_ini = np.zeros((N-1,)).tolist()
w_T_attic_l_ini = np.zeros((N-1,)).tolist()
w_T_im_l_ini = np.zeros((N-1,)).tolist()

R_win_l_ini = (R_win * np.ones((N+1))).tolist()
R_w2_l_ini = (R_w2 * np.ones((N+1))).tolist()
R_attic_l_ini = (R_attic * np.ones((N+1))).tolist()
R_im_l_ini = (R_im * np.ones((N+1))).tolist()
R_roof_l_ini = (R_roof * np.ones((N+1))).tolist()


C_in_l_ini = (C_in * np.ones((N+1))).tolist()
C_w_l_ini = (C_w * np.ones((N+1))).tolist()
C_attic_l_ini = (C_attic * np.ones((N+1))).tolist()
C_im_l_ini = (C_im * np.ones((N+1))).tolist()
C1_l_ini = (C1 * np.ones((N+1))).tolist()
C2_l_ini = (C2 * np.ones((N+1))).tolist()
C3_l_ini = (C3 * np.ones((N+1))).tolist()

x_initial = vertcat(*T_ave_l_ini, *T_wall_l_ini, *T_attic_l_ini, *T_im_l_ini,  *R_win_l_ini, *R_w2_l_ini,  *R_attic_l_ini, *R_im_l_ini,  *R_roof_l_ini, *C_in_l_ini, *C_w_l_ini, *C_attic_l_ini, *C_im_l_ini, *C1_l_ini, *C2_l_ini, *C3_l_ini, v_l_ini, *w_T_ave_l_ini, *w_T_wall_l_ini, *w_T_attic_l_ini, *w_T_im_l_ini)

# Creating Lower/Upper bounds on Variables and Equations
x_lb = vertcat(*T_ave_lb, *T_wall_lb, *T_attic_lb, *T_im_lb,  *R_win_lb, *R_w2_lb,  *R_attic_lb, *R_im_lb,  *R_roof_lb, *C_in_lb, *C_w_lb, *C_attic_lb, *C_im_lb, *C1_lb, *C2_lb, *C3_lb, v_lb, *w_T_ave_lb, *w_T_wall_lb, *w_T_attic_lb, *w_T_im_lb )

x_ub = vertcat(*T_ave_ub, *T_wall_ub, *T_attic_ub, *T_im_ub,  *R_win_ub, *R_w2_ub,  *R_attic_ub, *R_im_ub,  *R_roof_ub, *C_in_ub, *C_w_ub, *C_attic_ub, *C_im_ub, *C1_ub, *C2_ub, *C3_ub, v_ub, *w_T_ave_ub, *w_T_wall_ub, *w_T_attic_ub, *w_T_im_ub )

G_lb = vertcat(*Eq_T_ave_lb, *Eq_T_wall_lb, *Eq_T_attic_lb, *Eq_T_im_lb, *Eq_y_lb)

G_ub = vertcat(*Eq_T_ave_ub, *Eq_T_wall_ub, *Eq_T_attic_ub, *Eq_T_im_ub, *Eq_y_ub)

# Solving NLP Problem
NLP_Solution = NLP_Solver(x0 = x_initial, lbx = x_lb, ubx = x_ub, lbg = G_lb, ubg = G_ub)

#----------------------------------------------------------------Solution Analysis ----------------------------------------------------------------------#
#--------------------------------------------------------------------------------------------------------------------------------------------------------#

## Getting the Solutions
NLP_Sol = NLP_Solution['x'].full().flatten()

T_ave_sol = NLP_Sol[0:N]
T_wall_sol = NLP_Sol[N:2*N]
T_attic_sol = NLP_Sol[2*N:3*N]
T_im_sol = NLP_Sol[3*N:4*N]

R_win_sol = NLP_Sol[4*N:5*N]
R_w2_sol = NLP_Sol[5*N:6*N]
R_attic_sol = NLP_Sol[6*N:7*N]
R_im_sol = NLP_Sol[7*N:8*N]
R_roof_sol = NLP_Sol[8*N:9*N]

C_in_sol = NLP_Sol[9*N:10*N]
C_w_sol = NLP_Sol[10*N:11*N]
C_attic_sol = NLP_Sol[11*N:12*N]
C_im_sol = NLP_Sol[12*N:13*N]
C1_sol = NLP_Sol[13*N:14*N]
C2_sol = NLP_Sol[14*N:15*N]
C3_sol = NLP_Sol[15*N:16*N]

##########################################
v_sol = NLP_Sol[16*N:17*N]
w_T_ave_sol = NLP_Sol[17*N:18*N-1]
w_T_wall_sol = NLP_Sol[18*N-1:19*N-2]

## Simulation Plotting
# Setting Figure Size
plt.rcParams['figure.figsize'] = [Plot_Width, Plot_Height]

# Plotting Figures
plt.figure()

# Plotting  States
plt.plot(time_vector, T_ave_sol[0:-1].transpose(), label=r'$\ T_ave $ $(rads)$')
plt.plot(time_vector, T_wall_sol[0:-1].transpose(), label=r'$\ T_wall = y$ $(rads/s)$')
plt.plot(time_vector, T_attic_sol[0:-1].transpose(), label=r'$\ T_attic $ $(rads)$')
plt.plot(time_vector, T_im_sol[0:-1].transpose(), label=r'$\ T_im = y$ $(rads/s)$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('States '+ r'$(x)$', fontsize=12)
plt.title('Building States - NLP Solution ' + r'x', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting Figures
plt.figure()

# Plotting  Parameters - R_win -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, R_win_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$R_win$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(R_win)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$R_win$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - R_w2 -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, R_w2_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$R_w2$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(R_w2)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$R_w2$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - R_attic -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, R_attic_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$R_attic$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(R_attic)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$R_attic$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - R_im -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, R_im_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$R_im$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(R_im)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$R_im$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - R_roof -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, R_roof_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$R_roof$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(R_roof)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$R_roof$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)



# Plotting  Parameters - C_in -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, C_in_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$C_in$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(C_in)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$C_in$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - C_w -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, C_w_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$C_w$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(C_w)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$C_w$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - C_attic -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, C_attic_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$C_attic$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(C_attic)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$C_attic$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - C_im -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, C_im_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$C_im$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(C_im)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$C_im$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - C1 -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, C1_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$C1$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(C1)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$C1$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - C2 -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, C2_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$C2$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(C2)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$C2$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Parameters - C3 -  of Nonlinear System
plt.subplot(211)
plt.plot(time_vector, C3_sol*np.ones((np.shape(time_vector)[0],1)), label=r'$C3$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(C3)$', fontsize=12)
plt.title('Building Parameter - NLP Solution '+ r'$C3$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)



# Plotting Figures
plt.figure()

# Plotting  Variables - v -  of Nonlinear System
plt.subplot(311)
plt.plot(time_vector, v_sol[0:-1].transpose(), label=r'$v$ $(rads/s)$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Variable '+ r'$(v)$', fontsize=12)
plt.title('Building Variable - NLP Solution '+ r'$v$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Variables - w_T_ave -  of Nonlinear System
plt.subplot(312)
plt.plot(time_vector, w_T_ave_sol, label=r'$w_{T_{ave}}$ $(rads)$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Variable '+ r'$(w_{T_{ave}})$', fontsize=12)
plt.title('Building Variable - NLP Solution '+ r'$w_{T_{ave}}$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Variables - w_T_wall -  of Nonlinear System
plt.subplot(313)
plt.plot(time_vector, w_T_wall_sol, label=r'$w_{T_{wall}}$ $(rads/s)$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Variable '+ r'$(w_{{T_{wall}})$', fontsize=12)
plt.title('Building Variable - NLP Solution '+ r'$w_{{T_{wall}}$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Variables - w_T_attic -  of Nonlinear System
plt.subplot(312)
plt.plot(time_vector, w_T_attic_sol, label=r'$w_{T_{attic}}$ $(rads)$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Variable '+ r'$(w_{T_{attic}})$', fontsize=12)
plt.title('Building Variable - NLP Solution '+ r'$w_{T_{attic}}$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)

# Plotting  Variables - w_T_im -  of Nonlinear System
plt.subplot(313)
plt.plot(time_vector, w_T_im_sol, label=r'$w_{T_{im}}$ $(rads/s)$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Variable '+ r'$(w_{{T_{im}})$', fontsize=12)
plt.title('Building Variable - NLP Solution '+ r'$w_{{T_{im}}$', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)